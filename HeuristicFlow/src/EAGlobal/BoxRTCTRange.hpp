...
class BoxDynamicRange {
 public:
  ...
  inline Var_t& min() { return _minV; }

  inline Var_t& max() { return _maxV; }

  inline const Var_t& min() const { return _minV; }

  inline const Var_t& max() const { return _maxV; }

  inline void setMin(const Var_t _m) { _minV = _m; }

  inline void setMax(const Var_t _m) { _maxV = _m; }
...
class BoxDynamicRange<Scalar_t, Size, BoxShape::SQUARE_BOX, DVO> {
 public:
  ...
  inline Scalar_t min() const { return _minS; }

  inline Scalar_t max() const { return _maxS; }

  inline Scalar_t& min() { return _minS; }

  inline Scalar_t& max() { return _maxS; }

  inline void setMin(Scalar_t s) { _minS = s; }

  inline void setMax(Scalar_t s) { _maxS = s; }
