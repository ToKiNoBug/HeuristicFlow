 ...
 *
 * \note If the number of objectives can't be fixed at compile time, you can use
 * `Eigen::Dynamic`(-1) to `ObjNum`. But other negative numbers and 0 aren't allowed. Mention that
 * Pareto optimality is a concept for multi-objective problems, using 1 for `ObjNum` is not allowed
 * as well. You will see static assertion failure if invalid value of `ObjNum` is assigned to
 * template parameter.
 */
...
struct Pareto {
   ...
   * \return true A is not worse than B on all objectives AND A has at least one objective that is
   * better than B.
   * \return false A doesn't dominate B
   */
  inline static bool isStrongDominate(const Fitness_t* A, const Fitness_t* B) noexcept {
